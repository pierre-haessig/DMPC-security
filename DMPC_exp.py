        ...
        for i_u in range(k_max):
            ...
            for n_hor in range(N):
                for j in range(m):
                    ...
                    Bj = (dt / Cth[j])
                    B_Textj = (dt / tau_th[j])
                    T_res[(k+n_hor+1) * m + j] = Aj * T_res[(k+n_hor) * m + j] + Bj * U[(k+n_hor) * m + j] + B_Textj * T_mod[(k+n_hor) * m + j]
...
if __name__ == '__main__':
    ...
    T_init = np.array([10, 10], dtype=float)
            ## Thermal Resistance
    Rth = np.array([50, 100], dtype=float)
            ## Thermal Capacity
    ...
    pb['T_init'] = T_init
    U, T_res, L, cost, J_u = optim_decen(pb, 1.5, 1.0e-1)
